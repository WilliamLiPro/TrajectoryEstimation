...
padding = params.padding;   %初始化
output_sigma_factor = params.output_sigma_factor;   %初始化
...
translation_model_max_area = params.translation_model_max_area; %初始化
nScales = params.number_of_scales;  %初始化, estimate, train
nScalesInterp = params.number_of_interp_scales; %初始化，estimate
scale_step = params.scale_step; %初始化
scale_sigma_factor = params.scale_sigma_factor;%初始化
scale_model_factor = params.scale_model_factor;%初始化
scale_model_max_area = params.scale_model_max_area;%初始化
interpolate_response = params.interpolate_response;%estimate
num_compressed_dim = params.num_compressed_dim;%初始化，train


s_frames = params.s_frames;%存储图像
pos = floor(params.init_pos);%目标位置，各个都用
target_sz = floor(params.wsize * params.resize_factor); %当前目标尺寸，为时变量

visualization = params.visualization;%可视化参数
...
if nScales > 0
    ...
    max_scale_dim = strcmp(params.s_num_compressed_dim,'MAX');  %最大尺寸维度 train 用
    if max_scale_dim
